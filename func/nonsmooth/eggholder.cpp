...
const double lb[] = {-512, -512};   // lower bound of the actual function range for each dimension
const double ub[] = {512, 512};     // upper bound
...
double get_obj(const double *x) {
    ...
    return 959.64066272085096898081 - ((x2+47)*sin(sqrt(fabs(x2+47+x1/2.0))) + x1*sin(sqrt(abs(x1 - (x2 + 47)))));
}
...
void get_obj_subg(const double *x, double *ret) {
    ...
    double abs1 = x2 + 47 + 0.5 * x1;
    double abs2 = x1 - x2 - 47;

    if (abs1 >= 0 && abs2 >= 0) {
        ret[0] = -sin(sqrt(abs2)) - x1*cos(sqrt(abs2)) / (2*sqrt(abs2)) - 0.25*(x2+47)*cos(sqrt(abs1)) / sqrt(abs1);
        ret[1] = -sin(sqrt(abs1)) + x1*cos(sqrt(abs2)) / (2*sqrt(abs2)) - (x2+47)*cos(sqrt(abs1)) / (2*sqrt(abs1));
