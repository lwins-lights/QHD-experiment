...
void sgd(const int dim, const int len, const double L, const double T, 
          const double dt, const double stepsize, const int par, 
          const double noise_level, const int max_sample_number) {
    /*
    ...
    */
    ...
    int num;
    if (max_sample_number != 0) {
        num = max_sample_number;
    } else {
        num = pow(len, dim);
    }
    ...
    //cout << "[DEBUG] " << noise_level * sqrt(dt) / sqrt(dim) << endl;
    
    /* 
     * run SGD with len^dim different initial values in the hypercube
     * switch to random samples if max_sample_number > 0
     */
    if (max_sample_number == 0) {
        ...
        for (int id = 0; id < num; id++) {
            for (int j = 0; j < dim; j++) {
                x[id][j] = y[id][j] = (uniform(gen) - 0.5) * 2 * L;
            }
        }
    }
    ...
    printf("\n");
    printf("SGD simulator runtime = %.5f s\n", time_ed - time_st);

    /* save results */
    npz_save("../result/sgd.npz", "expected_potential", pot, {(unsigned int) num_steps}, "w");
    npz_save("../result/sgd.npz", "probability_at_minimum", prob_at_min, {(unsigned int) num_steps}, "a");
    npz_save("../result/sgd.npz", "T", &T, {1}, "a");
    npz_save("../result/sgd.npz", "dt", &dt, {1}, "a");
    npz_save("../result/sgd.npz", "par", &par, {1}, "a");
    npz_save("../result/sgd.npz", "len", &len, {1}, "a");
    npz_save("../result/sgd.npz", "dim", &dim, {1}, "a");
    npz_save("../result/sgd.npz", "noise_level", &noise_level, {1}, "a");
}

int main(int argc, char **argv)
{
    if (argc != 6 && argc != 7) {
        perror("Expected arguments: ./sgd <len> <T> <dt> <par> <noise_level> [max_sample_number]");
        exit(EXIT_FAILURE);
    }
    ...
    const double noise_level = stod(argv[5]);
    int max_sample_number = 0;
    if (argc == 7) {
        max_sample_number = stoi(argv[6]);
    }
    ...
    sgd(dim, len, L, T, dt, 2 * L * fd_frac, par, noise_level, max_sample_number);
}